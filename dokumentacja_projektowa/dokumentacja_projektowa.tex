...
\begin{itemize}
    ...
    \item \textbf{Główne sekcje szablonu:}
        ...
        \end{itemize}
    \item \textbf{Kod szablonu:}
    \begin{lstlisting}[language=Python, caption=Szablon \texttt{profile.html}]
PROFILE_SECTION (ID = "profile", CLASS = "container-fluid mt-2"):
    HEADER:
        - TITLE: "My Data" (centered)
        - DESCRIPTION: "Here you can view and update your data." (centered, muted text)

    PERSONAL_INFO_ROW (CLASS = "row gy-4"):
        PERSONAL_INFO_COLUMN:
            ICON: "bi bi-person-circle" (size: fs-1)
            LABEL: "Full Name"
            VALUE: user.first_name + " " + user.last_name
        PERSONAL_INFO_COLUMN:
            ICON: "bi bi-telephone" (size: fs-1)
            LABEL: "Phone Number"
            VALUE: user.phone_number
        PERSONAL_INFO_COLUMN:
            ICON: "bi bi-calendar" (size: fs-1)
            LABEL: "Date of Birth"
            VALUE: user.birthday (formatted: "Y-m-d")
        ACTION_BUTTON_COLUMN:
            BUTTON (CLASS = "btn btn-outline-primary btn-lg w-100", DATA = modal trigger):
                - ICON: "bi bi-pencil"
                - TEXT: "Edit"
        \end{lstlisting}
    ...
    \begin{lstlisting}[language=Python, caption=Modal do edycji danych osobowych]
MODAL (ID = "editProfileModal", CLASS = "modal fade", ATTRIBUTES = tabindex, aria-labelledby, aria-hidden):
    MODAL_DIALOG (CLASS = "modal-dialog modal-lg"):
        MODAL_CONTENT (CLASS = "modal-content"):
            FORM (METHOD = POST):
                CSRF_TOKEN
                MODAL_HEADER:
                    - TITLE: "Edit Your Details"
                    - CLOSE_BUTTON: (CLASS = "btn-close", DATA = dismiss modal)
                MODAL_BODY:
                    FORM_GROUP:
                        - LABEL: "First Name"
...
\begin{itemize}
    ...
    \item \textbf{Kod szablonu:}
    \begin{lstlisting}[language=Python, caption=Szablon \texttt{add\_address.html}]
ADDRESS_FORM:
    FORM (METHOD = POST):
        CSRF_TOKEN
        LOOP over form_fields AS field:
            FORM_GROUP:
                - LABEL: field.label
                - INPUT:
                    - TYPE: text
        ...
        FORM_ACTIONS (CLASS: "card"):
            CARD_BODY:
                - LINK:
                    - HREF: cart_detail URL
                    - TEXT: "Back"
                - BUTTON:
                    - TYPE: submit
                    - TEXT: "Save Address"
...
\begin{itemize}
    ...
    \item \textbf{Działanie:}
        ...
        \end{itemize}
    \item \textbf{Kod szablonu:}
    \begin{lstlisting}[language=Python, caption=Szablon \texttt{address\_selection.html}]
ADDRESS_SELECTION_FORM:
    FORM (METHOD = POST):
        CSRF_TOKEN
        LOOP over addresses AS address:
            ADDRESS_CARD_WRAPPER:
                - CLASS: "address-default" IF address.id == default_address_id
                - RADIO_BUTTON:
                    - NAME: "selected_address"
                    - VALUE: address.id
                    - ID: "address-{address.id}"
                    - CHECKED: IF address.id == default_address_id
                - LABEL (FOR address.id):
                    ADDRESS_CARD_CONTENT:
                        - LINE: address.street (bold)
                        - LINE: address.city, address.postal_code
    ...
    JAVASCRIPT:
        - FILE: manage_address_selection.js
        \end{lstlisting}
\end{itemize}
...
\begin{lstlisting}[language=Python, caption=Szablon \texttt{payment.html}]
FORM:
    method = POST
    action = payment_form URL
    csrf_token()
...
\begin{lstlisting}[language=JavaScript, caption=Funkcja \texttt{payment.html}]
<script>
    document.addEventListener('DOMContentLoaded', function () {
...
\begin{lstlisting}[language=Python, caption=Szablon \texttt{blik\_payment.html}]
SECTION: Blik Payment
    HEADER:
        TITLE: "Enter Blik Code"
        DESCRIPTION: "To confirm the payment, enter the 6-digit code generated in your bank's mobile application."
    BODY:
        FORM:
            method = POST
            csrf_token()
...
class AllProductsView(CategoriesMixin, ListView):
    ...
    \item \subsubsection*{Szablony związane z widokiem AllProductsView}
        ...
        \begin{lstlisting}[language=Python, caption=Pseudocode for \texttt{all\_products.html}]
PRODUCT_LIST_SECTION:
    CSRF_TOKEN
    LOOP over object_list AS product:
        PRODUCT_CARD:
            LIKE_BUTTON:
                ...
                - ICON:
                    IF user is authenticated:
                        IF user likes product: FULL_HEART_ICON
                        ELSE: EMPTY_HEART_ICON
                    ELSE:
                        IF product ID in session likes: FULL_HEART_ICON
                        ELSE: EMPTY_HEART_ICON
...
class FavoritesListView(CategoriesMixin, ListView):
    ...
    \begin{lstlisting}[language=Python, caption=Szablon \texttt{favorites.html}]
FAVORITES_SECTION:
    CSRF_TOKEN
    LOOP over liked_products AS product:
        PRODUCT_CARD:
            LIKE_BUTTON:
                - CLASS: "like-btn"
                - ID: "like-btn-{product.id}"
...
\begin{lstlisting}[language=Python, caption=Szablon \texttt{category\_products.html}]
CATEGORY_PRODUCTS_PAGE:
    FILTER_BAR:
        FORM:
            - METHOD: GET
            - INPUT: "min_price" (placeholder = "Min Cena", value = min_price)
            - INPUT: "max_price" (placeholder = "Max Cena", value = max_price)
            - SELECT: "sort_by"
                - OPTION: "default" (selected if sort_by == "default")
                - OPTION: "price" (selected if sort_by == "price")
            - BUTTON: "Apply Filters"
    PRODUCT_LIST:
        LOOP over products AS product:
            PRODUCT_CARD:
                - TITLE: product.name
                - PRICE: product.price
    PAGINATION:
        IF products.has_previous:
            LINK: Previous page ("?page={products.previous_page_number}")
        CURRENT_PAGE:
            - TEXT: "Page {products.number} of {products.paginator.num_pages}"
        IF products.has_next:
            LINK: Next page ("?page={products.next_page_number}")
    \end{lstlisting}
...
\begin{itemize}
    ...
    \begin{lstlisting}[language=Python, caption=Szablon \texttt{header\_categories.html}]
HEADER_CATEGORIES:
    NAVBAR:
        TABLE:
            ROW:
                COLUMN (CLASS = "nav-item"):
                    - ID: "categories-toggle"
                    - TITLE: "Categories"
                    - LINK: "Categories" (ICON: fa-list)

    CATEGORY_LIST (ID = "header_categories"):
        LOOP over categories AS category:
            CATEGORY_ITEM:
                - DATA_INDEX: forloop.counter0
                - NAME: category.name
                - LINK: category_products URL (ID = category.id)

                IF category.subcategories exist:
                    SUBCATEGORY_LIST:
                        LOOP over subcategories AS subcategory:
                            SUBCATEGORY_ITEM:
                                - NAME: subcategory.name
                                - LINK: category_products URL (ID = subcategory.id)
        \end{lstlisting}
\end{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Fragment kodu:}
    \begin{lstlisting}[language=Python, caption=Szablon \texttt{left\_categories.html}]
LEFT_CATEGORIES_SECTION (ID = "left-categories"):
    CONTAINER:
        ASIDE:
            CATEGORY_LIST:
                LOOP over categories AS main_category:
                    MAIN_CATEGORY_ITEM:
                        - CLASS: "active" IF main_category.id == category.id
                        - NAME: main_category.name
                        - LINK: category_products URL (ID = main_category.id)

                    IF main_category.subcategories exist:
                        SUBCATEGORY_LIST:
                            LOOP over subcategories AS subcategory:
                                SUBCATEGORY_ITEM:
                                    - NAME: subcategory.name
...
\begin{itemize}
    ...
    \textbf{Kod metody:}
    \begin{lstlisting}[language=Python, caption=Metoda \texttt{Cart.get\_total\_price()}]
    ...
    \textbf{Kod metody:}
    \begin{lstlisting}[language=Python, caption=Metoda \texttt{CartItem.get\_total\_price()}]
    def get_total_price(self):
    ...
    \textbf{Logika:}
    \begin{itemize}
        \item Metoda mnoży cenę jednostkową produktu 
            \subsubitem (\texttt{self.product.price}) przez ilość (\texttt{self.quantity}).
...
\begin{lstlisting}[language=Python, caption=Metoda \texttt{get\_queryset()} w \texttt{CartDetailView}]
def get_queryset(self):
    if self.request.user.is_authenticated:
...
\begin{itemize}
    ...
    \item \textbf{Kod metody:}
\begin{lstlisting}[language=Python, caption=Metoda \texttt{get\_context\_data()} w \texttt{CartDetailView}]
def get_context_data(self, **kwargs):
...
\begin{itemize}
    ...
    \item \textbf{Kod:}
\begin{lstlisting}[language=JavaScript, caption=Funkcja \texttt{getCookie}]
...
\begin{itemize}
    ...
    \item \textbf{Kod:}
\begin{lstlisting}[language=JavaScript, caption=Funkcja \texttt{remove-from-cart}]
...
\begin{itemize}
    ...
    \item \textbf{Kod:}
\begin{lstlisting}[language=JavaScript, caption=Funkcja \texttt{update-cart-item}]
...
\begin{itemize}
    ...
    \item \textbf{Kod:}
\begin{lstlisting}[language=JavaScript, caption=Funkcja \texttt{add-to-cart}]
...
\begin{itemize}
    ...
    \item \textbf{Kod:}
\begin{lstlisting}[language=JavaScript, caption=Funkcja \texttt{updateTotalAmount}]
...
\begin{itemize}
    ...
    \item \textbf{Kod:}
\begin{lstlisting}[language=JavaScript, caption=Funkcja obsługująca \texttt{item-quantity}]
...
\begin{lstlisting}[language=Python, caption={Pobranie koszyka Usera}]
...
\begin{lstlisting}[language=Python, caption={Walidacja danych zamówienia}]
order_session = request.session.get('order_session', {})
...
\begin{lstlisting}[language=Python, caption={Tworzenie zamówienia}]
...
\begin{lstlisting}[language=Python, caption={Tworzenie konwersacji z adminem}]
...
\begin{itemize}
    ...
    \item \textbf{Kod:}
    \begin{lstlisting}[language=Python, caption={\texttt{get\_order\_status}}]
...
\begin{itemize}
    ...
    \item \textbf{Kod:}
\begin{lstlisting}[language=Python, caption={\texttt{simulate\_status\_update}}]
...
\begin{itemize}
    ...
    \item \textbf{Kod:}
\begin{lstlisting}[language=Python, caption={\texttt{track\_previous\_status}}]
...
<p>Laczna kwota: <strong>{{ order.total_amount|floatformat:2 }} zl</strong></p>
        ...
        \begin{lstlisting}[language=HTML, caption={Szablon texttt{order\_detail.html}}]
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{lstlisting}[language=HTML, caption={Szablon \texttt{order\_list.html}}]
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{lstlisting}[language=HTML, caption={\texttt{email/order\_confirmation\_email.html}}]
<ul>
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{lstlisting}[language=JavaScript, caption={Funkcja \texttt{order\_detail.js}}]
form.addEventListener('submit', function (event) {
    event.preventDefault();
...
\textbf{Kod:}
\begin{lstlisting}[language=Python, caption={Model \texttt{Rate}}]
...
\textbf{Kod:}
\begin{lstlisting}[language=Python, caption={Widok \texttt{rate\_product}}]
@csrf_exempt
...
\textbf{Kod:}
\begin{lstlisting}[language=Python, caption=Szablon \texttt{order\_detail.html}]
IF can_rate:
    RATING_SECTION:
        HEADER: "Rate this product"
        FORM (POST to rate_product URL with product ID):
            CSRF_TOKEN
            HIDDEN_INPUT: new_rate = true
            INPUT: rating (1-5, required)
...
% 
% UserRecommendedProductInteraction
% 
\clearpage
...
% 
% Funkcjonalność edytowania opinii
% 
\clearpage
...
\begin{lstlisting}[language=Python, caption=Szablon \texttt{rating\_list.html}]
    LOOP over ratings AS rating:
        REVIEW_CARD:
            - USERNAME: rating.user.username
            - RATING_STARS:
                LOOP over "12345" AS i:
                    IF rating.value >= forloop.counter:
                        STAR: filled (bi-star-fill, text-warning)
                    ELSE:
                        STAR: empty (bi-star, text-warning)
            - COMMENT: rating.comment
            - CREATED_AT: rating.created_at
    
            IF user is authenticated AND rating.user == user:
                EDIT_BUTTON:
...
\begin{lstlisting}[language=Python, caption=Szablon \texttt{product\_detail.html}]
EDIT_RATING_MODAL:
    FORM (POST to rate_product URL):
        CSRF_TOKEN
        HIDDEN_INPUT: rating_id
        INPUT: value (1-5, required)
        TEXTAREA: comment
        BUTTONS:
            - SAVE: Submit changes
            - CANCEL: Close modal
...
\begin{lstlisting}[language=Python, caption=Pseudokod przycisku reakcji]
REACTION_BUTTON:
    - CLASS: "btn like-btn btn-outline-secondary"
    - ID: "like-btn-{product.id}"
    - DATA: product ID
    ICON:
        IF user is authenticated:
            IF user likes product: FULL_HEART_ICON (fas fa-heart)
            ELSE: EMPTY_HEART_ICON (far fa-heart)
        ELSE:
            IF product ID in session likes: FULL_HEART_ICON (fas fa-heart)
            ELSE: EMPTY_HEART_ICON (far fa-heart)
\end{lstlisting}
...
\begin{lstlisting}[language=Python, caption=Szablon \texttt{messages\_list.html}]
MESSAGES_LIST:
    LIST (CLASS: "list-group list-group-flush"):
        LOOP over conversations AS conversation:
            LIST_ITEM (DATA: conversation.id):
                IF conversation.participant EXISTS:
                    TEXT: "Conversation with {conversation.participant}"
                ELSE:
                    TEXT: "General conversation"
    CHAT_MESSAGES_SECTION (ID: "chat-messages")
\end{lstlisting}
...
\begin{lstlisting}[language=Python, caption=Szablon \texttt{index.html}]
HTML_TEMPLATE:
    HEAD:
        - METADATA:
            - CHARSET: "UTF-8"
    ...
    BODY:
        ...
        INCLUDE: slider_recommended.html
        INCLUDE: slider_favorites.html

        IF messages EXIST:
            LOOP over messages AS message:
                POPUP_MESSAGE:
                    - CONTENT: message

        JSON_SCRIPT:
            - ID: "liked-product-ids"
            - CONTENT: liked_product_ids
